...
def calculate_model_accuracy(model, X_test, y_test):
    if isinstance(model, LinearRegression):
        ...
        test_pred = model.predict(X_test)
        if len(test_pred.shape) == 1:  # Handling 1D array
            test_pred = (test_pred >= 0.5).astype(int)
        else:
    ...
    try:
        ...
        if model_choice == 'Linear Regression':
            ...
            return render_template('index.html',
                                   ...
                                   model_name='Linear Regression', accuracy=accuracy)
        elif model_choice == 'Perceptron':
            prediction, _ = predict(features, perceptron_model)
            accuracy = perceptron_accuracy
            return render_template('index.html', prediction_text=f'Diabetes Prediction (1: true, 0: false): {prediction}', model_name='Perceptron',
                                   accuracy=accuracy)
        elif model_choice == 'DFNN':
            prediction, _ = predict(features, dfnn_model)
            prediction_class = 1 if prediction >= 0.5 else 0
            return render_template('index.html',
                                   prediction_text=f'Diabetes Prediction (1: true, 0: false): {prediction_class}', model_name='DFNN')
        else:
